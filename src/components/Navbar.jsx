...
export default function Navbar() {
  return (
    <nav className="fixed w-full z-50 bg-[#040504]/90 backdrop-blur-lg border-b border-[#393938]/40 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20 sm:h-24 transition-all duration-300">
          
          {/* Logo Image with Antigravity */}
          <motion.div 
            animate={{ y: [0, -6, 0] }}
            transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
            className="flex-shrink-0 flex items-center cursor-pointer will-change-transform"
          >
            {/* The image is imported natively via Astro or built-in bundler */}
            <img 
              src="/src/assets/logo barbosa chancho.png" 
              alt="Logo Casa Barbosa" 
              className="h-16 sm:h-20 w-auto object-contain drop-shadow-[0_0_15px_rgba(239,197,36,0.3)] transition-transform duration-500 hover:scale-105"
            />
          </motion.div>

          {/* Desktop Menu */}
          <div className="hidden sm:ml-6 sm:flex sm:space-x-10 font-serif text-lg sm:text-xl">
